...
class Run(object):
    ...
    def _set_run_name(self):
        """Sets run name."""
        cfg = self._cfg
        cfg.run_name = f"{cfg.env.id}.arp+.{cfg.run_prefix}.{cfg.seed}"

    def _set_log_path(self):
        """Sets paths to log directories."""
        log_dir = Path(self._cfg.log_root_dir).expanduser() / self._cfg.run_name
        self._cfg.log_dir = str(log_dir)
        self._cfg.video_dir = str(log_dir / "video")
        self._cfg.demo_dir = str(log_dir / "demo")
        self._cfg.ckpt_dir = str(log_dir / "ckpt")
        self._cfg.replay_dir = str(log_dir / "replay")
