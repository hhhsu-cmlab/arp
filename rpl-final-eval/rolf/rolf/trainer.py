...
class Trainer(object):
    ...
    def __init__(self, cfg):
        ...
        # self._agent = self._get_agent_by_name(cfg)(
        #     cfg.rolf, ob_space, ac_space
        # )

        self._agent = self.get_arp(cfg)
    ...
    def train(self):
        """Trains an agent."""
        cfg = self._cfg
        self._agent.build(training=True, device=cfg.device)
    ...
    def evaluate(self):
        ...
        cfg = self._cfg
        self._agent.build(training=False, device=cfg.device)
        self._agent.load('/home/by4212/Desktop/hw/rpl/furniture-bench/model_28000.pth')
        self._agent.eval()
        # ckpt_info = self._load_ckpt(cfg.init_ckpt_path, cfg.ckpt_num)
        # step = ckpt_info.get("step", 0)
        step = 0 
    ...
    def get_arp(self, cfg):
        ...
        net = PolicyNetwork(cfg.model.hp, cfg.env, render_device=f"{cfg.device}").to(cfg.device)
        agent = Policy(net, cfg.model.hp, log_dir=cfg.log_dir)

        # agent.build(training=True, device=cfg.device)
        return agent

    def _get_agent_by_name(self, cfg):
