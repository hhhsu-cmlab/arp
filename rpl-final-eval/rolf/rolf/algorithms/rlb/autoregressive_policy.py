...
class Policy:
    ...
    def load(self, model_path):
        ...
        model = self._network
        checkpoint['model_state'] = {key.replace('resnet', 'vision_encoder.resnet'): value
                                    for key, value in checkpoint['model_state'].items()}
        checkpoint['model_state'] = {key.replace('dino_v2', 'vision_encoder.dino_v2'): value
                                    for key, value in checkpoint['model_state'].items()}
        checkpoint['model_state']['vision_encoder.fc.weight'] = checkpoint['model_state']['fc.weight']
        checkpoint['model_state']['vision_encoder.fc.bias'] = checkpoint['model_state']['fc.bias']
    ...
    def get_runner(self, cfg, env, env_eval):
        ...
        from rolf.rolf.algorithms.rollout import RolloutRunner
