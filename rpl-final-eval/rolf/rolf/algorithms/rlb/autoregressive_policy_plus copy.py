...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training: # this is true if self.train() is called
            ...
            batch_size = 1
            wrist_camera_rgb_image = batch[-2][1]
            front_camera_rgb_image = batch[-1][1]
            batch = batch[:-2]
            ...
            # state_name = ['state_ee_position', 'state_ee_quaternion', 'state_ee_velocity', 'state_ee_angular_velocity', 'state_gripper_width', 'action_ee_delta_position', 'action_ee_delta_quaternion', 'action_gripper_range']
            # token_tensors = [batch[token_name] for token_name in state_name]
            token_tensors = []
            prompt_tk_ids = []
            for b in batch:
                token_tensors.append(b[1].unsqueeze(0))
                prompt_tk_ids.append(self.policy.token_name_2_ids[b[0]])
            # print(*token_tensors)
            prompt_tk_vals = cat_uneven_blc_tensors(*token_tensors) 
            prompt_tk_ids = torch.tensor(prompt_tk_ids)

            # prompt_tk_ids = torch.tensor([self.policy.token_name_2_ids[token_name] for token_name in state_name])
            # prompt_tk_vals = prompt_tk_vals.squeeze(0)
            prompt_tk_ids = prompt_tk_ids.unsqueeze(0).unsqueeze(-1)
            prompt_tks = torch.cat([prompt_tk_vals, prompt_tk_ids], dim=-1)
            prompt_tks = prompt_tks.to(device)

            # prompt_seq = torch.zeros([1, 0, 5], device=device)

            future_tk_chk_ids = [dict(chk_id=chk_id, tk_id=self.policy.token_name_2_ids[tk_name])
                                 for chk_id, tk_name in zip(range(len(batch), len(batch)+3), ['action_ee_delta_position', 'action_ee_delta_quaternion', 'action_gripper_range'])]
            
            result = self.policy.generate(prompt_tks, future_tk_chk_ids, match_layer='self', 
                                                    sample=False,  block_attn_directions=[(len(batch), len(batch)+3)],  
