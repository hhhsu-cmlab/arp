...
class RolloutRunner(object):
    ...
    def run_episode(self, record_video=False):
        ...
        # obs_history = {k: np.array(v, dtype=type(v)) if not isinstance(v, dict) else v for k, v in ob_next.items()}
        ...
        # prev_ac = torch.zeros([8],dtype=torch.float32)
        # # Rollout one episode.
        # while not done:
        #     ob = ob_next
        #     prev_ac = prev_ac.numpy()

        #     batch["state_ee_position"].append(ob['robot_state']['ee_pos'])
        #     batch["state_ee_quaternion"].append(ob['robot_state']['ee_quat'])
        #     batch["state_ee_velocity"].append(ob['robot_state']['ee_pos_vel'])
        #     batch["state_ee_angular_velocity"].append(ob['robot_state']['ee_ori_vel'])
        #     batch["state_gripper_width"].append(ob['robot_state']['gripper_width'])
        #     batch["action_ee_delta_position"].append(prev_ac[0:3])
        #     batch["action_ee_delta_quaternion"].append(prev_ac[3:-1])
        #     batch["action_gripper_range"].append([prev_ac[-1]])
            
        #     batch["wrist_camera_rgb"] = ob['color_image1']
        #     batch["front_camera_rgb"] = ob['color_image2']

        #     # change to torch tensor
        #     prepped_data = batch
        #     for k, v in batch.items():
        #         prepped_data[k] = torch.tensor(np.array(v), dtype=torch.float32)
        #         print(prepped_data[k].shape)

        obs_history = []
        
        prev_ac = np.zeros([8],dtype=np.float32)
        # Rollout one episode.
        while not done:
            ob = ob_next
            obs_history.append(("state_ee_position", ob['robot_state']['ee_pos'][0]))
            obs_history.append(("state_ee_quaternion", ob['robot_state']['ee_quat'][0]))
            obs_history.append(("state_ee_velocity", ob['robot_state']['ee_pos_vel'][0]))
            obs_history.append(("state_ee_angular_velocity", ob['robot_state']['ee_ori_vel'][0]))
            obs_history.append(("state_gripper_width", ob['robot_state']['gripper_width'][0]))
            obs_history.append(("action_ee_delta_position", prev_ac[0:3]))
            obs_history.append(("action_ee_delta_quaternion", prev_ac[3:-1]))
            obs_history.append(("action_gripper_range",[prev_ac[-1]]))
            ...
            for i in range(len(obs_history)):
                prepped_data.append((obs_history[i][0], torch.tensor(np.array(obs_history[i][1]), dtype=torch.float32).unsqueeze(0)))
            prepped_data.append(("wrist_camera_rgb", torch.tensor(np.array(ob['color_image1']), dtype=torch.float32)))
            prepped_data.append(("front_camera_rgb", torch.tensor(np.array(ob['color_image2']), dtype=torch.float32)))
            # prepped_data["wrist_camera_rgb"] = torch.tensor(np.array(ob['color_image1']), dtype=torch.float32)
            # prepped_data["front_camera_rgb"] = torch.tensor(np.array(ob['color_image2']), dtype=torch.float32)

            # prepped_data = {k: torch.tensor(np.array(v, dtype=np.float32), device=torch.device('cuda'))[None, ...] if not isinstance(v, dict) else torch.tensor(np.array(v, dtype=np.float32), device=torch.device('cuda')) 
            #                 for k, v in obs_history.items()}
            # Sample action from policy.
            ac = agent.act_f(prepped_data)
            ...
            rollout.add(dict(ob=ob, ob_next=ob_next, ac={'ac': ac['ac'].cpu()}, done=done, rew=reward))
